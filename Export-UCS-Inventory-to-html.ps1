...
$EXPORTFILENAME = "UCS_Inventory_"+"$UCS"+"_"+$LOGDATE+".html"
#Create Info Folder
...
##Clean up Old HTML Files Prior to Starting
Write-Host "Deleting Old HTML Exports"
$DatetoDelete = $CurrentDate.AddMonths($MONTHSTOKEEP)
Get-ChildItem $EXPORTFOLDER -Filter *.html | Where-Object { $_.LastWriteTime -lt $DatetoDelete } | Remove-Item -Confirm:$false
Write-Host "Completed deleting Old HTML Exports"
...
GenerateReport $UCS $OutFile $UCSCredentials
Write-Host (Get-Date -format "MMM-dd-yyyy_HH-mm-ss")
Write-Host "-----------------------------------------------------------------------------------------------------------------------"
}
